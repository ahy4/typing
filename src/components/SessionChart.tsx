...
export function SessionChart({ sessions }: Props) {
  if (sessions.length < 2) {
    return <p className="text-gray-600 text-sm text-center">Play 2+ sessions to see trends.</p>;
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      <h3 className="text-sm uppercase tracking-widest text-gray-500">Speed Trend (last 30 sessions)</h3>
      <svg width={w} height={h} className="w-full" viewBox={`0 0 ${w} ${h}`}>
        ...
        {/* line */}
        <path d={pathD} fill="none" stroke="#00ffff" strokeWidth="2" style={{ filter: "drop-shadow(0 0 4px #00ffff88)" }} />
        {/* dots */}
        {points.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r="2.5" fill="#00ffff" opacity="0.8" />
        ))}
        {/* labels */}
        <text x={pad} y={h - 1} fill="#444" fontSize="8" fontFamily="monospace">
          0
        </text>
        <text x={w - pad - 16} y={pad + 8} fill="#444" fontSize="8" fontFamily="monospace">
          {Math.round(maxWpm)}
        </text>
      </svg>
    </div>
