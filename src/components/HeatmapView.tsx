...
function aggregateFromSessions(sessions: SessionRecord[]): Map<string, KeyData> {
...
function aggregateFromReplay(replay: ReplayData): Map<string, KeyData> {
  ...
  for (const ev of replay.events) {
    ...
    if (!ev.correct) {
      combined.set(ev.key, { ...existing, count: existing.count + 1, errors: existing.errors + 1 });
    } else {
      const correctInterval = lastCorrectTime > 0 ? ev.time - lastCorrectTime : interval;
...
function getColor(key: string, combined: Map<string, KeyData>, maxCount: number) {
  const d = combined.get(key);
  if (!d || d.count === 0) return { bg: "#151515", text: "#2a2a2a", border: "#1e1e1e" };

  const errRate = d.errors / d.count;
  const avgMs = d.count - d.errors > 0 ? d.totalMs / Math.max(d.count - d.errors, 1) : 500;
  // usage intensity 0..1
...
export function HeatmapView({ sessions, replay }: Props) {
  ...
  const maxCount = Math.max(1, ...Array.from(combined.values()).map((d) => d.count));
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        {ROWS.map((row, ri) => (
          <div key={ri} className="flex gap-1.5">
            ...
            {row.map((key) => {
              ...
              const avgMs = d && correctCount > 0 ? Math.round(d.totalMs / correctCount) : null;
              return (
                <div
                  key={key}
                ...
                >
                  ...
                  {errPct !== null && errPct > 0 && (
                    <span
                      ...
                      style={{ color: errPct > 30 ? "#ffaaaa" : errPct > 15 ? "#ffdd88" : "#aaa", opacity: 0.85 }}
                    >
                      {errPct}%
                    </span>
                  )}
                  {avgMs !== null && (errPct === null || errPct <= 15) && (
                    <span className="text-[8px] leading-tight mt-0.5 text-gray-400 opacity-70">{avgMs}ms</span>
                  )}
                </div>
              );
            })}
      ...
      {/* Legend */}
      <div className="mt-2 p-3 rounded border border-gray-800 flex flex-wrap gap-4 text-xs font-mono" style={{ background: "#0a0a0a" }}>
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded flex items-center justify-center text-xs font-bold"
            style={{ background: "rgba(220,40,40,0.85)", color: "#ffaaaa", border: "1px solid rgba(255,80,80,0.6)" }}>a</div>
          <span style={{ color: "#ff8888" }}>エラー多 (30%+)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded flex items-center justify-center text-xs font-bold"
            style={{ background: "rgba(200,120,20,0.6)", color: "#ffdd88", border: "1px solid rgba(255,160,30,0.5)" }}>a</div>
          <span style={{ color: "#ffdd88" }}>エラーやや多 (15-30%)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded flex items-center justify-center text-xs font-bold"
            style={{ background: "rgba(0,240,255,0.65)", color: "rgba(80,255,255,0.95)", border: "1px solid rgba(0,240,255,0.5)" }}>a</div>
          <span style={{ color: "#00ffee" }}>速い (&lt;150ms)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded flex items-center justify-center text-xs font-bold"
            style={{ background: "rgba(0,140,170,0.25)", color: "rgba(80,200,255,0.55)", border: "1px solid rgba(0,140,170,0.3)" }}>a</div>
