...
```bash
node scripts/chunk-sentences.mjs --count <N>
```

- stdout に **配列の配列（チャンクの配列）** が出力される。外側の配列の各要素が 1 チャンク、各チャンクは `[{index, jp, kana}, ...]` の形式
- index は sentences.toml 全体での 0 始まり通し番号
- exit code は常に 0

出力を JSON.parse してチャンクリストを取得する。チャンク数 `C = チャンクリスト.length`。
...
```bash
node scripts/find-similar-sentences.mjs --output gomi/similar_islands.json
```

- `gomi/similar_islands.json` に JSON 配列の配列が書き出される（外側の配列の各要素が 1 島、各島は `[{index, jp, kana}, ...]` の形式）
- 島とは「kana 編集距離がしきい値以下のペアで繋がった連結成分」
- exit code は常に 0

`Read` で `gomi/similar_islands.json` を読み込んで JSON.parse し、島リストを取得する。
