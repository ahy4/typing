...
**既存ファイルへの `Write` 前には必ず `Read` が必要**（Claude Code のツール制約）。`gomi/` の各ファイルを `Write` する前に `Read` を試みること。ファイルが存在しない場合は `Read` がエラーになるがそのまま `Write` できる。ファイルが存在する場合は `Read` 後に `Write` する。`gomi/` 配下は一時ファイルなので、`Read` が失敗した場合（前回の残骸でファイルが大きい等）でもそのまま `Write` してよい。
