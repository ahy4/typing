...
3. **JP の長さ**: 
   - **短い単語（1～3文字）**: 「犬」「猫」「風」「紐」など漢字1～2文字の単語（小学6年生までの常用漢字）
   - **短め（4～10文字）**: 短い文
   - **普通（11〜18文字）**: 標準的な文
   - **長め（19〜25文字）**: やや長い文
...
専用エージェント `sentence-generator`（`.claude/agents/sentence-generator.md`）を使う。`gen-prompt.md` の読み込みは不要。
...
- `<JP_LENGTH_INSTRUCTION>` → 以下のいずれか（`jp.length` の目安、厳守不要）:
  - 短い単語: `jp.length の目安は 1～2 です（犬、猫、風、紐など小学6年生までの常用漢字）`
  - 短め: `jp.length の目安は 4～10 です`
