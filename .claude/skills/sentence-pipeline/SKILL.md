...
## Phase 1: Generate（7ラウンド逐次実行）

以下の順序で 7 ラウンドを**完全に逐次実行**する（前のラウンドが完了してから次を開始）。
...
各ラウンド完了後に「追記件数」を記録する。7 ラウンド分を合算し **合計追記件数 T** とする。

### 各ラウンドの手順

各ラウンドで以下のステップ G1〜G5 を実行する。
...
`gomi/` が存在しない場合は先に `mkdir -p gomi` を実行。
...
ステップ G2 を通過した文をまとめて `gomi/sentences_to_validate.json` に `Write` で書き出す:
...
#### ステップ G4: sentences.toml に追記

1. exit 1 の場合: stdout を JSON.parse して errors を除いた有効文リストを `Write` で `gomi/sentences_to_validate.json` に上書き保存する
2. `Bash` で以下を実行:
   ```bash
...
#### a. sentences.toml の修正

`src/lib/sentences.toml` を `Read` ツールで読み込み、対象エントリを特定する。

**削除の場合**: `Edit` ツールで該当の `[[sentences]]` ブロック（空行 + `[[sentences]]` + `jp = ...` + `kana = ...` の4行）をまるごと削除する。

**修正の場合**: `Edit` ツールで該当の `kana = "..."` 行のみを正しい値に置き換える。
