...
**途中で処理を中断することは絶対に禁止。** トークン使用量・コンテキスト長・「効率化のため」などの理由で G1〜Phase 3 の途中で完了報告をしてはいけない。V5/V6 まで必ず実行してコミットしてから完了報告する。
...
**Agent 並列起動のバッチ分割は絶対に禁止。**

- 「1つのメッセージで同時起動」と書かれた箇所では、**必ず全エージェントを1回のメッセージ送信で起動する**
- 「効率化」「安全のため」「様子見」などの理由でバッチを分割してはいけない
- 10個でも30個でも、指定された数をそのまま1メッセージに含める
- 分割すると並列実行の恩恵を失い、前バッチ完了待ちが発生してパイプライン全体が遅くなる
...
```
G1フェーズ: 全7ラウンドの合計 10 バッチを 1 メッセージで一括起動
     ↓ 全完了後
リトライ: 失敗バッチ（全ラウンド合算）を 1 メッセージで一括再実行
     ↓ 全完了後
G2フェーズ: parse → 全有効バッチの validator（最大 30 個）を 1 メッセージで一括起動
     ↓ 全完了後
...
全7ラウンドの **合計 10 バッチ**（R1〜R3 各2個 + R4〜R7 各1個）を **1 つのメッセージで同時起動**する。10個のAgent呼び出しを1メッセージに含めること（分割禁止）。
...
全7ラウンドの全有効バッチの合計を M_total とし、**3×M_total 個のサブエージェントを 1 つのメッセージで同時起動**する（最大 30 個）。3×M_total 個を1メッセージに含めること（分割禁止）。
...
このスクリプトは全バッチの `_v1/_v2/_v3` 結果ファイルを読み込み、OR ロジックで除外セットを確定し、各バッチの集計結果を `gomi/gen_results/aggregated_<R>_<i>.json`（`[{"index": N, "jp": "..."}, ...]` 形式）に書き出す。出力の `Aggregated N batches — M total excluded entries` から LLM-reject の合計件数を記録する。

続けて、Bash で以下を実行して各ラウンドの G3 入力ファイルを生成する:

```bash
node scripts/prepare-g3-inputs.mjs gomi/gen_batches gomi/gen_results gomi/
```

このスクリプトは LLM-reject を除外した有効文を各ラウンドの `gomi/sentences_to_validate_<R>.json` に書き出す。**親エージェントはファイルを Read する必要はない。**
...
1. `gomi/sentences_to_validate_<R>.json` が存在しない場合、または空配列 `[]` の場合はこのラウンドをスキップして「有効文 0 件」と記録する。
2. ```bash
   node --experimental-strip-types scripts/validate-sentences.ts --filter gomi/sentences_to_validate_<R>.json 2>/dev/null
   ```
   `--filter` モードはエンジン-reject をファイルからその場で除外して常に exit 0 で終わる。出力（`Filtered N engine-reject entries → M remain` または `All M entries valid`）をラウンドの記録として残す。
...
2. `Bash` で `npm run gen >/dev/null 2>&1` を実行して `src/lib/generated/sentences.json` を再生成する。
...
全 C チャンクのサブエージェントを **1 つのメッセージで同時起動**（C 個を1メッセージに含めること。分割禁止）:
...
全 I 対象島のサブエージェントを **1 つのメッセージで同時起動**（I 個を1メッセージに含めること。分割禁止）:
